...
def train_generate(few, neg_num, dataset_path, batch_size,train_tasks,ent2id,rel2id, id2ent, id2rel, e1rel_e2, rel2candidates):
	...
	while True:
		...
		if len(query_trip) < batch_size:
			query_trip_pos = [random.choice(query_trip) for _ in range(batch_size)]
		else:
			query_trip_pos = random.sample(query_trip, batch_size)

		support_pair = [[pair[0],pair[2]] for pair in support_trip]
		query_pair_pos = [[pair[0],pair[2]] for pair in query_trip_pos]

		query_left = [triple[0] for triple in query_pair_pos]
		query_right = [triple[1] for triple in query_pair_pos]

		false_left = []
		false_right = []
		for triple in query_trip_pos:
			e_h = id2ent[triple[0]]
			rel = id2rel[triple[1]]
			e_t = id2ent[triple[2]]
			neg_sample_num = 0
			while neg_sample_num < neg_num:
		...
		yield (support_pair, query_pair_pos, one_tomany_train, candidates_id),(support_left, support_right, query_left, query_right, false_left, false_right)
